...
os.environ["CUDA_VISIBLE_DEVICES"] = "0"  # 93.90
import abc
...
class marn(NeuralNetwork):
    def __init__(self, config, adj, original_adj):
        ...
        self.text_fc = nn.Linear(in_features=768, out_features=300)  ## ViT-B/32
    ...
    def forward(self, X_tid, X_text, X_text_content):
        with torch.no_grad():
            X_text_clip = self.text_tokenizer(X_text_content, padding=True, truncation=True, max_length=512,
...
def load_dataset():
    ...
    X_dev_content = []
    X_test_content = []
    with open(pre + "/weibo.train", 'r', encoding='utf-8') as input:
        for line in input.readlines():
            tid, content, label = line.strip().split("\t")
            X_train_content.append(content)
    with open(pre + "/weibo.dev", 'r', encoding='utf-8') as input:
        for line in input.readlines():
            tid, content, label = line.strip().split("\t")
            X_dev_content.append(content)
    with open(pre + "/weibo.test", 'r', encoding='utf-8') as input:
        for line in input.readlines():
            tid, content, label = line.strip().split("\t")
...
def train_and_test(model):
    ...
    # 最优模型路径
    config['save_path'] = os.path.join(res_dir, "weibo2/model_weibo_filter_conv_batchsize64_meanpool2_wo_maxnorm")
    #
    ...
    nn.load_state_dict(torch.load(
        "exp_result/weibo2/model_weibo_filter_conv_batchsize64_meanpool2_wo_maxnorm"))

    # 最佳模型
    # nn.load_state_dict(torch.load(
    #     "exp_result/weibo2/model_weibo_best"))
