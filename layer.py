...
class OutputLayer_Filter(nn.Module):
    def __init__(self, d_in, d_hidden, n_classes, modal_num, seq_len, dropout=0.5, type='norm'):
        super(OutputLayer_Filter, self).__init__()
        self.seq_len = seq_len
        if type == 'norm':
            self.filter = FilterLayer(hidden_size=d_in // seq_len, seq_len=seq_len, prob=dropout)
        elif type == 'high':
            self.filter = HighPassFilterLayer(hidden_size=d_in // seq_len, seq_len=seq_len, prob=dropout)
        elif type == 'low':
            self.filter = LowPassFilterLayer(hidden_size=d_in // seq_len, seq_len=seq_len, prob=dropout)
        elif type == 'band':
            self.filter = BandPassFilterLayer(hidden_size=d_in // seq_len, seq_len=seq_len, prob=dropout)
        self.mlp_head = nn.Sequential(
            nn.LayerNorm(d_in),
            nn.Linear(d_in, d_hidden),
            nn.GELU(),
...
class VLTransformer_TypeFilterLayer(nn.Module):
    def __init__(self, input_data_dims, hyperpm,type):
        super(VLTransformer_TypeFilterLayer, self).__init__()
        self.hyperpm = hyperpm
        self.input_data_dims = input_data_dims
        self.d_q = hyperpm['n_hidden']
        self.d_k = hyperpm['n_hidden']
...
class HighPassFilterLayer(nn.Module):
    ...
    def forward(self, input_tensor):
        ...
        condition = torch.abs(freqs) < 0.1
        condition = condition.unsqueeze(0).unsqueeze(-1).cuda()
        x = x.masked_fill(condition, 0)
        sequence_emb_fft = torch.fft.irfft(x, n=seq_len, dim=1, norm='ortho')
        hidden_states = self.out_dropout(sequence_emb_fft)
        hidden_states = self.LayerNorm(hidden_states + input_tensor)
...
class BandPassFilterLayer(nn.Module):
    def __init__(self, seq_len, hidden_size, prob):
        super(BandPassFilterLayer, self).__init__()
        self.out_dropout = nn.Dropout(p=prob)
        self.LayerNorm = LayerNorm(hidden_size, eps=1e-12)
    ...
    def forward(self, input_tensor):
        ...
        # x = x * weight
        freqs = torch.fft.rfftfreq(seq_len)
        condition = (torch.abs(freqs) >= 0.1) & (torch.abs(freqs) <= 0.2)
        condition = condition.unsqueeze(0).unsqueeze(-1).cuda()
        x = x.masked_fill(condition, 0)
        sequence_emb_fft = torch.fft.irfft(x, n=seq_len, dim=1, norm='ortho')
        hidden_states = self.out_dropout(sequence_emb_fft)
...
#
#     # 将低频部分置零
#     freq_signal[torch.abs(freqs) < cutoff] = 0
#     return fft.ifft(freq_signal).real
#
#
...
#
#
# def band_pass_filter(signal, low_cutoff, high_cutoff):
#     """带通滤波器"""
#     freq_signal = fft.fft(signal)
#     freqs = fft.fftfreq(len(signal))
#
#     # 将不在范围内的频率部分置零
#     mask = (torch.abs(freqs) >= low_cutoff) & (torch.abs(freqs) <= high_cutoff)
#     freq_signal[~mask] = 0
#     return fft.ifft(freq_signal).real
...
class ContrastDrivenFeatureAggregation(nn.Module):
    ...
    def forward(self, x, fg, bg):
        ...
        B, H, W, C = x.shape
        v = self.v(x).permute(0, 3, 1, 2)  # bchw
